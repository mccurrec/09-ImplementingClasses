...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.distance_traveled = self.distance_traveled + ((self.x - self.recent_x) ** 2 + (self.y - self.recent_y)
                                                           ** 2) ** (0.5)
        self.recent_x = self.x
        self.recent_y = self.y
    ...
    def closer_to(self, p2, p3):
        distance2 = ((self.x - p2.x) ** 2 + (self.y - p2.y) ** 2) ** (0.5)
        distance3 = ((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2) ** (0.5)
